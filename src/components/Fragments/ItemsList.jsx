                ...
                <tbody>
                    {items.map((item, index) => (
                        <tr key={item.id}>
                            ...
                            <th>
                                <Link to={`/product/edit/${item.id}`} className="button is-small is-info mr-1"><FaRegEdit /></Link>
                                {isDeleted ? <Link onClick={() => { activateProduct(item.id) }} className="button is-small is-danger">Activate </Link> : <button onClick={() => { deleteProduct(item.id) }} className="button is-small is-danger"><MdDelete /> </button>
                                }
                            </th>
                        </tr>
                    ))}
